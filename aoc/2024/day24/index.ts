...
const getSwappedWires = (gates: Gate[]) => {
  ...
  for (let i = 0; i < zCount - 1; i++) {
    ...
    if (!sumG || sumG.output !== z) {
      ...
      if (!sumG) {
        swaps.push(and2.output, xor1.output);
        [xor1.output, and2.output] = [and2.output, xor1.output];
